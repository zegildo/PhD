...
def getTimestampUnix(timestamp):
	...
	----------
	timestamp: string
		uma string no formato hh`h`mm dd-mm-yy
	...
	Examples
	--------
	>>> data = '12h32 14-11-2000'
	>>> getTimestampUnix(data)
	3265874189

	"""
	timestamp = timestamp.translate(None, '|')
	unixTimesTamp = int(datetime.datetime.strptime(timestamp, '%Hh%M %d-%m-%Y').strftime("%s"))
...
python infomoneyCrawler /mercados/noticia/1869
						----------------------
								argv[1]

parallel -X python infomoneyCrawler /mercados/acoes-e-indices/noticia/{ids} ::: {1869..final} &
parallel -X python infomoneyCrawler /mercados/noticia/{ids} ::: {1869..final} &
parallel -X python infomoneyCrawler /mercados/politica/noticia/{ids} ::: {1869..final} &
...
if __name__ == "__main__":
	...
	for parteDaURLComId in partesDaUrlComId:
		...
		URL = 'http://www.infomoney.com.br/mercados%s'%(parteDaURLComId)
		try:
			response = requests.get(URL)  
			body = html.fromstring(response.text)
			titulo = getStringSemEspacamento(body.xpath('//hgroup/h1/text()')[0])
