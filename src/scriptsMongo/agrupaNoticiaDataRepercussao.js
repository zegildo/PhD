...
function generenateDatesBetween(startDate, stopDate){
    ...
    while (currentDate <= stopDate) {
        ...
        dateMap[dataFormatada]={"G1":{"qt":0,"repercussao":[]},"FOLHASP":{"qt":0,"repercussao":[]},"ESTADAO":{"qt":0,"repercussao":[]}};
	...
	function (doc){
		...
		if(["ESTADAO","FOLHASP","G1"].indexOf(doc.subFonte) > -1){
			...
			if(dateKey in mapaDeDatas){
				...
				mapaDeDatas[dateKey][doc.subFonte]["repercussao"].push(doc.repercussao);
			}else{
...
for (var data in mapaDeDatas){
	...
	print(diaDaSemana+","+mes+","+dia+","+ano+","+"G1,"+mapaDeDatas[data]["G1"]["qt"]);
	for(var rep in mapaDeDatas[data]["G1"]["repercussao"]){
		print(mapaDeDatas[data]["G1"]["repercussao"][rep])
	}
	print(diaDaSemana+","+mes+","+dia+","+ano+","+"FOLHASP,"+mapaDeDatas[data]["FOLHASP"]["qt"]);
	for(var rep in mapaDeDatas[data]["FOLHASP"]["repercussao"]){
		print(mapaDeDatas[data]["FOLHASP"]["repercussao"][rep])
	}
	print(diaDaSemana+","+mes+","+dia+","+ano+","+"ESTADAO,"+mapaDeDatas[data]["ESTADAO"]["qt"]);
	for(var rep in mapaDeDatas[data]["ESTADAO"]["repercussao"]){
		print(mapaDeDatas[data]["ESTADAO"]["repercussao"][rep])
	}
